...
public class XOA_MON_AN {
    ...
    public int XOA_MON_AN_XuLyPhanTom(String maDoiTac, String maMonAn, String maChiNhanh) {
        int check = 0;
        try {
            PreparedStatement pst = con.prepareStatement("{call USP_XoaMonAnPhanTomFix(?,?,?)}");
            pst.setString(1, maDoiTac);
            pst.setString(2, maMonAn);
            pst.setString(3, maChiNhanh);
            ResultSet rs = pst.executeQuery();
